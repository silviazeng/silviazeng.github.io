---
title: A semiconductor industry reader
date: 2026-08-26
tags:
  - AI
  - learning
  - history
  - interactive
  - chinese
description: "Three interactive pages on the chip industry — a short history told as five breakpoints, the geopolitics around TSMC told as chokepoints, and a clickable map of the whole value chain. Bilingual, with a hover note on every term."
---

Recently I tried to explain to family why one company in Taiwan prints most of the world's advanced chips, and found I couldn't do it without a whiteboard.

So I wrote the whiteboard down. Most chip-industry histories are invention timelines — who built what in which year — and they don't stick, because inventions don't explain structure. **The question that makes the industry legible is who holds the pricing power, and why it moves.** Asked that way, eighty years collapse into five breakpoints, and the last one is happening right now.

Three pages, each bilingual (中文 / EN), with a hover note on every term of art:

<figure style="max-width:576px;margin:1.75rem auto;">
  <img src="/static/semis/img/map-diagram.png" alt="The value-chain map: EDA and IP license into chip design, equipment and HBM feed in from above, wafers flow rightward to packaging, systems and cloud, with an amber loop showing hyperscalers designing their own chips" style="width:100%;border-radius:12px;box-shadow:0 6px 24px rgba(0,0,0,0.35);display:block;">
  <figcaption style="font-size:0.82rem;color:#6f5a3c;text-align:center;margin-top:0.6rem;line-height:1.4;">The full map — wafers flow left to right; the amber dashed loop is the hyperscalers designing their own chips. Badges mark where each jurisdiction lands.</figcaption>
</figure>

<figure style="max-width:576px;margin:1.75rem auto;">
  <img src="/static/semis/img/map-tooltip.png" alt="The same map with a hover note open on CoWoS, explaining TSMC's advanced packaging and why it is the real bottleneck of AI chip supply" style="width:100%;border-radius:12px;box-shadow:0 6px 24px rgba(0,0,0,0.35);display:block;">
  <figcaption style="font-size:0.82rem;color:#6f5a3c;text-align:center;margin-top:0.6rem;line-height:1.4;">Every stage, arrow and badge carries a note — here, why CoWoS packaging, not wafers, is the real bottleneck.</figcaption>
</figure>

<div style="display:flex;flex-wrap:wrap;gap:0.75rem;margin:1.25rem 0;">
  <a href="/static/semis/five-breakpoints.html" data-router-ignore style="flex:1 1 220px;padding:1.1rem 1.25rem;border-radius:12px;background:#1c2b1c;color:#a0d8a0;text-decoration:none;line-height:1.4;">
    <strong>Five Breakpoints</strong><br><span style="font-size:0.85rem;opacity:.85;">A short history of the semiconductor industry, told by where the money and pricing power moved.</span>
  </a>
  <a href="/static/semis/tsmc-geopolitics.html" data-router-ignore style="flex:1 1 220px;padding:1.1rem 1.25rem;border-radius:12px;background:#2b241c;color:#e2c98a;text-decoration:none;line-height:1.4;">
    <strong>Chokepoints, Not Chips</strong><br><span style="font-size:0.85rem;opacity:.85;">The geopolitics around TSMC: who can stop whom, where that grip is migrating, and what it costs.</span>
  </a>
  <a href="/static/semis/value-chain-map.html" data-router-ignore style="flex:1 1 220px;padding:1.1rem 1.25rem;border-radius:12px;background:#1c242b;color:#9cc3e0;text-decoration:none;line-height:1.4;">
    <strong>The Full Map</strong><br><span style="font-size:0.85rem;opacity:.85;">Every layer of the value chain and who plays where — filter by region, hover any player.</span>
  </a>
</div>

Start with the map if you want the picture, the history if you want the plot, the geopolitics if you want the fight. They cite their sources; figures are as of August 2026, and the estimates are flagged as estimates.
